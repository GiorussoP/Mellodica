...
public:
  ColliderComponent(Actor *owner, ColliderLayer layer,
                    ColliderType type = ColliderType::AABB,
                    bool isStatic = false, int updateOrder = 10);
  virtual ~ColliderComponent();

  // Check if two colliders intersect (virtual to allow different
  // implementations)
  virtual bool Intersect(const ColliderComponent &other) const = 0;

  // Detect collision and return penetration vector (virtual to allow different
  // implementations)
...
public:
  AABBCollider(Actor *owner, ColliderLayer layer, Vector3 offset, Vector3 size,
               bool isStatic = true, int updateOrder = 10);
  ~AABBCollider();
...
public:
  OBBCollider(Actor *owner, ColliderLayer layer, Vector3 offset, Vector3 size,
              bool isStatic = false, int updateOrder = 10);
  ~OBBCollider();
...
public:
  SphereCollider(Actor *owner, ColliderLayer layer, Vector3 offset,
                 float radius, bool isStatic = false, int updateOrder = 10);
  ~SphereCollider();
  ...
  Vector3 GetOffset() const { return mOffset; }
  float GetRadius() const; // Returns scaled radius
