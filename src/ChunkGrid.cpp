...
ChunkGrid::ChunkGrid(const Vector3& worldMin, const Vector3& worldMax, float cellSize)
: mWorldMin(worldMin)
, mWorldMax(worldMax)
, mCellSize(cellSize)
{
    ...
    std::cout << "ChunkGrid created: " << mGridWidth << "x" << mGridDepth 
              << " cells (" << totalCells << " total), cell size: " << cellSize << std::endl;
    std::cout << "World bounds: (" << mWorldMin.x << ", " << mWorldMin.z << ") to (" 
              << mWorldMax.x << ", " << mWorldMax.z << ")" << std::endl;
}
...
std::vector<Actor*> ChunkGrid::GetVisibleActors(const Vector3& cameraPos)
{
    ...
    if (!debugPrinted) {
        ...
        std::cout << "Grid dimensions: " << mGridWidth << "x" << mGridDepth << std::endl;
        ...
        std::cout << "Non-empty cells: " << nonEmptyCells << "/" << mCells.size() << std::endl;
        std::cout << "Max actors in single cell: " << maxActorsInCell << std::endl;
    ...
    */
    ...
    for (int dz = -1; dz <= 1; dz++)
    {
        for (int dx = -1; dx <= 1; dx++)
        {
            ...
            visibleActors.insert(visibleActors.end(), cellActors.begin(), cellActors.end());
        }
    }
...
void ChunkGrid::GetCellCoords(const Vector3& position, int& outX, int& outZ) const
{
...
int ChunkGrid::CoordsToIndex(int x, int z) const
{
    return z * mGridWidth + x;
}
