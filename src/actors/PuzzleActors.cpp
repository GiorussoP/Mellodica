...
ItemActor::ItemActor(Game* game) : Actor(game), mAnimTime(0.0f) {
    ...
    auto mc = new MeshComponent(this, *mesh);
    mc->SetColor(Vector3(1.0f, 1.0f, 0.0f));
    mc->SetBloomed(true);

    new SphereCollider(this, ColliderLayer::Entity, Vector3::Zero, 0.5f, false);
}
...
MusicButtonActor::MusicButtonActor(Game* game, int midiTarget)
    ...
    mCollider = new OBBCollider(this, ColliderLayer::Entity, Vector3::Zero, Vector3(0.5f, 0.5f, 0.5f), false);
}

void MusicButtonActor::OnCollision(Vector3 penetration, ColliderComponent* other) {
