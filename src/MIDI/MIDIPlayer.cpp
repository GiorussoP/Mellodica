...
void MIDIPlayer::loadSong(const char * filename,bool loop_enabled) {
        ...
        for (const auto& trackEvent : events) {
            ...
            if (event->getType() == MidiType::EventType::MidiEvent) {
                ...
                if (status == MidiType::MidiMessageStatus::NoteOn) {
                    ...
                    channels.at(channel).notes.push_back({current_seconds,true,midiEvent->getNote(),midiEvent->getVelocity()});
                } else if (status == MidiType::MidiMessageStatus::NoteOff) {
                    channels.at(channel).active = true;
                    channels.at(channel).notes.push_back({current_seconds,false,midiEvent->getNote(),midiEvent->getVelocity()});
    ...
    std::cout<<"\nLoaded "<<filename<<", song length: "<<song_length<<std::endl;
...
void MIDIPlayer::update(float dt) {
    ...
    for (int i = 0; i < 16; ++i) {
        ...
        while (channel.pos < channel.notes.size() && time >= channel.notes.at(channel.pos).start) {
            if (!paused && channel.active) {
                //std::cout<<"Time:"<<time<<", event: "<<channel.notes.at(channel.pos).start<<' '<<channel.notes.at(channel.pos).note<<" "<<channel.notes.at(channel.pos).on<<'\n';
                if (channel.notes.at(channel.pos).on)
                    SynthEngine::startNote(i,channel.notes.at(channel.pos).note,channel.notes.at(channel.pos).velocity);
                else
