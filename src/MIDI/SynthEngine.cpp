...
void SynthEngine::init(const char* soundfont_path, const char* audio_driver) {
    ...
    fluid_settings_setnum(settings, "synth.sample-rate", 32000); // try 32000 or 22050
...
std::vector<std::pair<std::string,SoundPreset>> SynthEngine::getSoundPresets() {

    fluid_sfont_t* sfont = fluid_synth_get_sfont_by_id(synth, sfid);

    std::vector<std::pair<std::string,SoundPreset>> presets;

    fluid_sfont_iteration_start(sfont); fluid_preset_t* preset;
    while ((preset = fluid_sfont_iteration_next(sfont))) {
        std::pair<std::string,SoundPreset> p ={fluid_preset_get_name(preset),{fluid_preset_get_banknum(preset),fluid_preset_get_num(preset)}};
...
void SynthEngine::setChannels(const std::vector<SoundPreset>& presets) {
    ...
    for (int ch = 0; ch < presets.size(); ++ch) {
        ...
        if (fluid_synth_program_select(synth, ch, sfid, preset.bank_num, preset.num) != 0) {
            std::cerr
              << "Failed to bind channel " << ch
              << " to bank " << preset.bank_num
              << ", num " << preset.num << "\n";
        }
...
void SynthEngine::startNote(unsigned int ch,unsigned int note, unsigned int velocity) {
...
void SynthEngine::testSoundFont() {
    ...
    for (auto preset : presets) {
        std::cout<<"Testing "<<preset.first<<": ("<<preset.second.bank_num<<'/'<<preset.second.num<<")..."<<std::endl;
